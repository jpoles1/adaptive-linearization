...
exports.AdaptiveLinearization = void 0;
var PI = Math.PI;
var TAU = PI * 2;
var DEFAULT_OPTS = {
...
var AdaptiveLinearization = /** @class */ (function () {
    function AdaptiveLinearization(consumer, opts) {
         ...
         */
        this.svgPathIterator = function (segment, index, curX, curY) {
            ...
            switch (command) {
                case "M":
                    for (i = 1; i < segment.length; i += 2) {
                        x = segment[i];
                        y = segment[i + 1];
                        _this.consumer(curX, curY, x, y, index, ["M", x, y]);
                        curX = x;
                        curY = y;
                    }
                    break;
                case "L":
                    for (i = 1; i < segment.length; i += 2) {
                        x = segment[i];
                        y = segment[i + 1];
                        _this.consumer(curX, curY, x, y, index, ["L", x, y]);
                        curX = x;
                        curY = y;
                    }
                    break;
                case "H":
                    ...
                    for (i = 1; i < segment.length; i += 2) {
                        x = segment[i];
                        _this.consumer(curX, curY, x, y, index, ["H", x]);
                        curX = x;
                    }
                    break;
                case "V":
                    ...
                    for (i = 1; i < segment.length; i += 2) {
                        y = segment[i];
                        _this.consumer(curX, curY, x, y, index, ["V", y]);
                        curY = y;
                    }
                    break;
                case "Z":
                    _this.consumer(curX, curY, curX, curY, index, ["Z"]);
                    break;
                case "Q":
                    short = true;
                ...
                case "C":
                    ...
                    for (i = 1; i < segment.length; i += step) {
                        ...
                        _this.linearize(x, y, x2, y2, x3, y3, x4, y4, index);
                        curX = x;
                        curY = y;
                    }
                    break;
        ...
        this.consumer = function (x1, y1, x2, y2, data, svgcmd) {
    ...
    AdaptiveLinearization.prototype.distanceTo = function (x1, y1, x2, y2) {
        var x = x2 - x1;
        var y = y2 - y1;
        return Math.sqrt(x * x + y * y);
    };
    AdaptiveLinearization.prototype.linearizeRecursive = function (x1, y1, x2, y2, x3, y3, x4, y4, data, level) {
        var cuspLimit = this.opts.cuspLimit;
        var curveColinearityEpsilon = this.opts.curveColinearityEpsilon;
        var curveAngleToleranceEpsilon = this.opts.curveAngleToleranceEpsilon;
        var angleTolerance = this.opts.angleTolerance;
        var distanceToleranceSquared = Math.pow((0.5 / this.opts.approximationScale), 2);
        ///////////////////////////////
        ...
        var da1, da2, k;
        switch ((d2 > curveColinearityEpsilon ? 2 : 0) +
            ...
            case 0:
                ...
                if (k === 0) {
                    ...
                    if (d2 <= 0)
                        d2 = this.distanceTo(x2, y2, x1, y1);
                    else if (d2 >= 1)
                        d2 = this.distanceTo(x2, y2, x4, y4);
                    else
                        d2 = this.distanceTo(x2, y2, x1 + d2 * dx, y1 + d2 * dy);
                    if (d3 <= 0)
                        d3 = this.distanceTo(x3, y3, x1, y1);
                    else if (d3 >= 1)
                        d3 = this.distanceTo(x3, y3, x4, y4);
                    else
                        d3 = this.distanceTo(x3, y3, x1 + d3 * dx, y1 + d3 * dy);
                }
                if (d2 > d3) {
    ...
    return AdaptiveLinearization;
}());
exports.AdaptiveLinearization = AdaptiveLinearization;
